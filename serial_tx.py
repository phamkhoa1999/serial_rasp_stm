...
def main():
    ...
    #rospy.Subscriber('/Ozawa',Twist,Callback)
    encoders = rospy.Publisher('Ozawa', Twist, queue_size=10)    
    ...
    while not rospy.is_shutdown():
        if flag_uart == 0x1:
            flag_uart = 0x0
     #   if enc_L < 0 and enc_R < 0:
    #        str_msg_tx = "%06.2f/%06.2fk" % (enc_L,enc_R)
   #     if enc_L > 0 and enc_R < 0:
  #          str_msg_tx = "+%05.2f/%06.2fk" % (enc_L,enc_R)
 #       if enc_L < 0 and enc_R > 0:
#	    str_msg_tx = "%06.2f/+%05.2fk" % (enc_L,enc_R)
 #       if enc_L >= 0 and enc_R >= 0:
#    	    str_msg_tx = "+%05.2f/+%05.2fk" % (enc_L,enc_R)
